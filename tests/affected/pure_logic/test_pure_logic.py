...
from doctrack.commands.affected import _find_direct_hits, _propagate, find_affected_docs
...
def test_find_affected_docs_no_changes():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        docs_dir.mkdir()
        config = Config({})
        with patch("doctrack.commands.affected.get_changed_files", return_value=[]):
...
def test_find_direct_hits_exact_match():
    ...
    hits, matches = _find_direct_hits(["src/module.py"], source_to_docs)
    assert doc1 in hits
...
def test_find_direct_hits_directory_match():
    ...
    hits, matches = _find_direct_hits(["src/booking/booking.module.ts"], source_to_docs)
    assert doc1 in hits
