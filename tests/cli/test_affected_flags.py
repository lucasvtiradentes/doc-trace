...
from doctrace.cli import main


def test_affected_requires_scope_flag():
    with patch.object(sys, "argv", ["doctrace", "affected", "docs/"]):
...
def test_affected_json_flag():
    with patch("doctrace.cli.affected.run", return_value=0) as run_mock:
        with patch.object(sys, "argv", ["doctrace", "affected", "docs/", "--last", "1", "--json"]):
            with pytest.raises(SystemExit) as exc:
                main()
    assert exc.value.code == 0
...
def test_affected_since_flag():
    with patch("doctrace.cli.affected.run", return_value=0) as run_mock:
        with patch.object(sys, "argv", ["doctrace", "affected", "docs/", "--since", "v1.0.0"]):
            with pytest.raises(SystemExit) as exc:
                main()
    assert exc.value.code == 0
